...
import torch
import numpy as np
from plpm import calculate_PLV, InverseModelingExpData, ModelWrapper
...
InvResults.to_csv('data/InvModeling/InvResults_Final.csv', index=True)

MtcResults = pd.DataFrame(index=NamesList,
                          columns=['$R^2_{Vlv}$', '$R^2_{Plv}$'],
                          data=AllMetrics[1:, :].numpy())

MtcResults.to_csv('data/InvModeling/MtcResults_Final.csv', index=True)
