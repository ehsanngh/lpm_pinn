...
plt.style.use(resource_stream('plpm', '/classes/paper.mplstyle').name)
...
class MainPlots:
    ...
    def plot_CASE_seperate(self,
        ...
        t_pinn = torch.linspace(0., 1., V_pinn.shape[0]) * base.Tc
        ...
        if V_numerical is not None:
            t_numerical = torch.linspace(0., 1., V_numerical.shape[0]) * base.Tc
    ...
    def plot_residuals(self, residuals):
        t_plot = torch.linspace(0., 1., residuals.shape[0]) * base.Tc
    ...
    def plot_loss(self, loss_file):
        loss = np.loadtxt(resource_stream(
            'plpm', '../data/loss/' + loss_file + '_Residuals.csv'),
            delimiter=",",
            skiprows=1)
        epochs = loss[:, 0]
        train_loss = loss[:, 1]
...
class InverseModelingPlots:
    ...
    def plot_VLV(self,
        ...
        if vlv_num is not None:
            t_numerical = torch.linspace(0., 1., vlv_num.shape[0]) * base.Tc
    ...
    def plot_PLV(self,
        ...
        t_pinn = torch.linspace(0., 1., plv_pinn.shape[0]) * base.Tc
        ...
        if plv_num is not None:
            t_numerical = torch.linspace(0., 1., plv_num.shape[0]) * base.Tc
    ...
    def plot_Pao(self,
        ...
        t_pinn = torch.linspace(0., 1., Pao_pinn.shape[0]) * base.Tc
        ...
        if Pao_num is not None:
            t_numerical = torch.linspace(0., 1., Pao_num.shape[0]) * base.Tc
